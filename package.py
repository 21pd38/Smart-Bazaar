...
def shopping():
    #Main title
    st.markdown("<h1 style='text-align: center; color: #FF0000; font-family: Georgia, serif; font-size: 80px; font-weight: bold; border: 5px solid #FF0000; padding: 20px; border-radius: 30px; background: linear-gradient(to right, #0A192F, #1C3456);'>SMART SALES</h1>", unsafe_allow_html=True)
...
def datasets():  
    ...
    # Define the options in the drop-down menu
    options = ['Default', 'Low to High prices', 'High to Low Prices', 'Average Rating']

    # Create a drop-down menu with the options
    selected_option = st.sidebar.selectbox('Sort by:', options)

    # Display the appropriate dataframe based on the selected option
    if selected_option == 'Default':
        st.write(amazon)
    elif selected_option == 'Low to High prices':
        st.write(amazonlowtohigh)
    elif selected_option == 'High to Low Prices':
        st.write(amazonhightolow)
    elif selected_option == 'Average Rating':
        st.write(amazonavgrating)
